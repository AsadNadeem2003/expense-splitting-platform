  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      try {
        ...
        const filtered = results.filter((u: any) => !currentMemberIds.has(u.id));
                      ...
                      {searchResults.map(u => (
                        <div key={u.id} className="flex justify-between items-center p-3.5 hover:bg-slate-50 transition-colors">
                          ...
                          <button 
                            ...
                            className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded-lg text-xs font-bold transition-all shadow-sm active:scale-95 flex-shrink-0 cursor-pointer"
                          >
                            Add
                          </button>
                        </div>
